...
- AI 提供完整代码、表结构、SQL query、DTO、接口或 service 方案时，必须同步解释字段 / 参数 / 方法的含义、谁创建、谁消费、使用时机和关键约束；不能只给代码或只给方向。
...
- 注释规则:
  ...
  - 接口, 结构体, 方法 必须添加注释
- 代码组织规则:
  - 接口必须放在文件最前面的类型声明区，先展示该文件依赖的能力边界。
  - 结构体定义后必须紧跟它的构造函数，便于先理解对象如何创建和校验依赖。
