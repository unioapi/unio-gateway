...
```text
当前主线：阶段 12 Capability Architecture（DEC-015）能力架构**代码层已收口**（用户授权方案 A，2026-06-09）：闸门默认 observe，capability 推断/档位/observe-enforce 闸门/观测审计/enforce 开关+三协议渲染/`/v1/models` cap-tags 全交付，已关闭 GAP-12-001/003/004/005/008/012。三类需外部前置的工作受控 deferred 阶段 13：`/console/v1/models`（GAP-12-006）、adapter 画像物化进真实 model 行（GAP-12-007，依赖 provisioning）、observe→enforce 实际切换 + 观察期 + 覆盖面复核（GAP-12-009/12-002）。全量 `go build`/`go vet`/`gofmt`/`go test`（含 DATABASE_URL DB 往返）全绿。
