...
```text
最新进展（2026-06-12）：OpenAI 官方 1P 上游**代码接入完成**——路线 C 去方言化（base 忠实输出 `max_completion_tokens` / `developer` role，两条 DeepSeek 塌缩下沉 `openai/deepseek` 层零回归）+ 注册 `(openai, openai)` adapter（复用 base，含 tokenizer）+ 黑盒经 OpenRouter OpenAI 协议端点 3 用例全过（忠实字段 200 接受、流式尾包 usage 正常）；官方直连模型实测被 OpenRouter 账户级 403（中国区 ToS）拦截，待官方 key 补冻结（见 providers/openai/upgrade-plan.md N6）。Anthropic 官方接入（beta 透传/tokenizer/注册）未开工。
