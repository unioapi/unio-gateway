...
2. 当前 text-only MVP 不假装支持这些能力；进入正式实现前必须先补稳 SSE parser，再设计 tool/multimodal DTO、adapter contract、stream delta、usage/billing 和 fallback 语义。
3. 下次继续逐章复盘时，优先判断阶段 5 的 `GAP-5-002` 是否现在处理，因为 `bufio.Scanner` stream parser 会阻碍 tool_calls 和大 chunk。
...
| 阶段 5 | [Adapter 边界](chapters/phase-05-adapter-boundary/STATUS.md) | partial | adapter 接口、OpenAI 非流式/流式、usage 映射和当前 HTTP DTO 可透传参数 contract 已完成，更稳健 SSE parser 未完成。 |
...
1. 本轮逐章复盘：下一步进入阶段 5 [TASK-5.03](chapters/phase-05-adapter-boundary/PLAN.md#task-5-03-openai-stream-adapter)，判断是否替换 `bufio.Scanner` stream parser，为 tool_calls、大 chunk 和 backpressure 做准备。
