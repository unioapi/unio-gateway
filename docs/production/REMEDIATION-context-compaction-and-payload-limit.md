...
| 属性 | 值 |
| --- | --- |
| 状态 | **已实施**（阶段 11 增量 TASK-11.18~11.24；[DEC-019](DECISIONS.md#dec-019-可配置请求体上限--compact-双路径native-透传--synthetic-降级)；审核 2026-06-18，Q1=128MB，Q2~Q7 按建议默认） |
| 创建日期 | 2026-06-16 |
| 实施日期 | 2026-06-18 |
...
| 新增 GAP（已写入） | [GAP-11-013](TODO_REGISTER.md#gap-11-013)（请求体上限，已关闭）、[GAP-11-014](TODO_REGISTER.md#gap-11-014)（compact 双路径，已关闭） |
...
- [x] Codex 长会话复现时，`/v1/responses/compact` 不再因 **< 几 MB** 的 body 在入口 413（默认上限 128MB；`httpx.DecodeJSON` 读 `MaxJSONBodyBytes()`）。
- [x] 构造 `> HTTP_MAX_JSON_BODY_MB` 的请求仍稳定 413，错误体为 OpenAI-compatible（`MaxBytesReader` + 既有 `ErrRequestBodyTooLarge`→413 渲染）。
- [x] `go test` 覆盖：默认配置、显式 env、非法值（`config_test.go`）+ 默认回退/抬高/收紧/负值回退（`json_test.go`）。
- [x] `.env.example` 已更新（`HTTP_MAX_JSON_BODY_MB=128` + 前置代理须 ≥ 此值说明）。
...
- [x] OpenAI/Codex 官方渠道：compact 请求到达上游 `/responses/compact`，响应原文透传（adapter `adapter_test.go` httptest 验证 URL `/v1/responses/compact` + body 逐字未改写 + raw 透传；service `compact_native_test.go` 验证 model 回显改写 + compaction item 保留）。
- [x] DeepSeek 渠道：SyntheticCompact 行为与原 `compact_response.go` 一致（`compact_input_tokens_test.go` 回归绿）。
- [x] 路由选路按 `adapter_key` 代码能力 `HasResponsesCompact`（与 DEC-018 一致，能力 key 作契约声明）；无能力渠道走 Synthetic，不误透传。
- [x] 两条路径均经 `runNonStream` → `AttemptRunner` 产生 `request_records` + `usage_records` + settlement（单测断言 settlement 各一次）。
- [x] [CAPABILITY_MATRIX.md](../chapters/phase-11-openai-responses-api/CAPABILITY_MATRIX.md) 已按 Native / Synthetic 两路径拆分 compact 声明。
...
| # | 问题 | 建议默认 | 你的决定（2026-06-18） |
| --- | --- | --- | --- |
| Q1 | `HTTP_MAX_JSON_BODY_MB` 生产默认值 | **64MB** | ✅ **128MB**（config 代码默认；env 可调小） |
| Q2 | Native compact 上游 404 时是否自动回落 Synthetic | **是**（打 warn 日志） | ✅ 是（`compactNativeFallback` 默认开 + `slog.Warn`） |
| Q3 | Synthetic 路径是否解析入站 `type:compaction` item | **否**（本阶段）；Codex 走 Native 时用官方 item | ✅ 否（本阶段） |
| Q4 | compact 能力 key 命名 | `responses.compact.native` + `responses.compact.synthetic` | ✅ 采纳（CAPABILITY_KEYS v1.1） |
| Q5 | 是否本阶段实现 `context_management.compact_threshold` | **否**，单独 follow-up | ✅ 否（deferred） |
| Q6 | 整改阶段归属 | 作为 **阶段 11 增量**（TASK-11.18+），不新开 phase | ✅ 阶段 11 增量（TASK-11.18~11.25） |
| Q7 | GAP-11-007 文档表述 | 拆为「Synthetic 永久限制」；Native 路径单独声明 | ✅ 已拆分（GAP-11-007=Synthetic 永久限制；Native 见 GAP-11-014） |
...
- [x] 问题描述与现场是否准确
- [x] 目标 / 非目标边界是否同意
- [x] 默认值（body 上限 128MB、回落默认开）已拍板（Q1/Q2）
- [x] 任务拆分粒度是否合适（TASK-11.18~11.25）
- [x] 同意作为阶段 11 增量而非新 phase（Q6）
- [x] 已更新 TODO_REGISTER（GAP-11-013/014）、PLAN（TASK-11.18~11.25）、STATUS、DECISIONS（DEC-019）、CAPABILITY_KEYS v1.1、CAPABILITY_MATRIX
