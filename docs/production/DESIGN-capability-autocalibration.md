...
> **SUPERSEDED（2026-06-23）**：本设计已被
> [DESIGN-capability-manual-declaration.md](DESIGN-capability-manual-declaration.md) 与
> [DEC-024](DECISIONS.md#dec-024-移除能力自动校正与能力闸门改为能力字典--人工声明) 取代。
> 能力自动校正将**彻底删除**（非停用）。下文仅作历史参考。

| 属性 | 值 |
| --- | --- |
| 状态 | **superseded**（→ DEC-024 / DESIGN-capability-manual-declaration） |
| 创建日期 | 2026-06-18 |
| 审核日期 | 2026-06-18 |
...
> ⚠️ **本节 v1 证据表已被 v2 取代**：见 [DESIGN-capability-evidence-v2.md](DESIGN-capability-evidence-v2.md) §4。v2 以 `request_attempts.used_capabilities`（响应 output item 命中）作为 tools.* / builtin 的强证据来源，废弃「tools.* 仅 finish_class=tool_use」主路径（Chat 保留作回退）。下表仅留作 v1 历史记录。
...
| TASK-H | P1 | 能力证据 v2（used_capabilities 闭环 + evidence 注册表 + OpenAI 首验） | [DESIGN-capability-evidence-v2.md](DESIGN-capability-evidence-v2.md) |
