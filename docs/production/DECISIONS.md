...
```text
GAP-7-007 继续作为公开计费前 release blocker。
settlement 成功重放检查和外部事务内 debit 幂等重入已完成，GAP-7-012 已在 2026-05-26 关闭。
后续进入 worker/recovery 线时，必须用已具备幂等边界的 settlement 重试。
```

## DEC-008 倍率属于运营配置，金额快照属于账务事实
...
```text
Unio API 底层账务不使用倍率作为最终计费事实。

倍率、分组折扣、批量调价可以作为后台运营配置方式，用于生成或解释客户售价。
结算时必须使用已经解析成明确金额的客户售价、provider/channel 成本价和请求级快照。
...
```text
倍率适合中转站后台运营：模型倍率、补全倍率和分组倍率能快速表达“哪个模型贵”“输出比输入贵多少”“不同用户组折扣多少”。

但倍率不是财务事实。只保存倍率会让历史账单、渠道成本、毛利、fallback 成本和价格变更后的复算都变得不可靠。
...
```text
TASK-7.20 先落地成本价与 cost snapshot，不先做倍率系统。

后续 Admin API 可以支持两种运营输入：

1. 直接填写 input/output/cached/reasoning 的明确金额。
2. 使用基准价、模型倍率、补全倍率、分组倍率生成明确金额。

无论后台用哪种输入方式，结算层只消费明确金额和快照，不在 settlement 时临时依赖倍率重算历史账单。
```
