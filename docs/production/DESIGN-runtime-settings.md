...
**域 = `Definition.Category` = key 前缀(首个 `.` 之前)**,注册表单测强制两者一致。四个域,
消费方互不加载对方的域;设置页按域分 Tab(参考 new-api 的设置页组织方式):

| 域 | 消费方 | 传播方式 | 例 |
|---|---|---|---|
| `gateway` | gateway 进程热路径 | settingsApplier 5s 推送 / 包级 atomic | 熔断器、限流默认等 6 组 |
| `anthropic` 等 Provider | gateway adapter | Provider 现读(store 三层缓存) | `anthropic.beta_policy` |
| `admin_backend` | admin 进程后端(运维聚合 service) | 每请求现读(本地 3s 缓存,无 applier) | `admin_backend.channel_health_thresholds` |
| `admin_frontend` | admin 前端(后端只存储+校验+下发) | 前端 react-query 拉取,与设置面板共用缓存,保存即失效重取 | `admin_frontend.dashboard_thresholds` |

注意:seed 与注册表仍全局共享(gateway 启动也会 seed `admin_*` 缺行,幂等无害);「域分开」
指**消费与展示**分开,不为每个进程拆注册表。个人偏好(分页大小/主题)不进本系统,归浏览器
localStorage。详见 `DESIGN-runtime-settings-batch2-domains.md`。
...
- `internal/service/appsettings/admin_backend_settings.go` — admin_backend 域(渠道健康分桶阈值)
- `internal/service/appsettings/admin_frontend_settings.go` — admin_frontend 域(仪表盘告警灯阈值,无 Go 消费方)
...
- 前端:`unio-admin/src/components/system/RuntimeSettingsPanel.tsx`(分域 Tab 编辑面板 + 偏离默认标记)、
  `unio-admin/src/hooks/useMetricThresholds.ts`(admin_frontend 域消费入口)
- 迁移:`000068_create_app_settings`、`000069_add_app_settings_description`、`000070_app_settings_duration_ms`
