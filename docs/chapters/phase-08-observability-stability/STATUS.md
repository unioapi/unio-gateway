...
| TASK-8.04 | done | retry/fallback（8.01）：`ProviderErrorClassifier` 按上游 category 判定 retryable，非流式未写出前可同模型 fallback，stream 写出后不 fallback。channel 熔断（本任务）：`channel_breaker.go` 进程内熔断器（固定窗口错误率 + open/half-open/closed 状态机），gateway 拿到 plan 后跳过 open channel 并按 category 记录健康（timeout/server_error/rate_limit/auth/permission 计故障，bad_request/canceled 不计）。默认启用，阈值可配置。核心 routing 保持纯查询，熔断过滤在 service 层。跨实例共享健康与后台手动恢复属阶段 11 admin。 |
