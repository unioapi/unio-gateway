...
状态：in_progress（admin-server，垂直切片推进；Slice 7 主线切片全部完成；Slice 8 看板经营驾驶舱 D1 决策层重构已交付）
...
## 已交付（Slice 7，M8 系统/任务/健康横切，2026-06-12）

结算补偿任务只读视图 + 系统级 channel 健康（派生）+ 同步任务只读复用，端到端打通并真实 PG 验证。

> **范围决策（本切片定调）**：纯只读、不新增业务事实/不加 migration。**熔断状态不可跨进程读**——`lifecycle/breaker.go` 明确为「gateway 进程内内存态、不依赖共享存储」，admin-server 是独立进程读不到实时电路；故 channel 健康沿用 M9 同源思路从区间内 `request_attempts` 成功率**派生分桶**（healthy≥0.95 / degraded≥0.80 / unhealthy / no_data），不动 gateway 热路径。把 DB runtime 熔断健康共享 + 后台手动恢复留作独立切片。同步任务复用 M5 已交付的 `GET /capability/sync-jobs`（前端只读展示，触发仍在能力中心）。

| 任务 | 状态 | 说明 |
| --- | --- | --- |
| TASK-13.08（M8 系统/任务/健康） | done | **结算补偿任务** `GET /system/settlement-recovery-jobs?status=&user_id=&from=&to=`（分页倒序，**列表 SQL 从不 SELECT `last_internal_error_detail`**，存储层即脱敏）+ `GET /system/settlement-recovery-jobs/{id}`（详情含归属/路由/金额快照/用量事实/最近错误；内部诊断详情仅 `?include_internal=true` 回显，与 M6 同策）。**系统级 channel 健康** `GET /system/channel-health?from=&to=`：per-channel `attempt_total/succeeded/failed/canceled` + 成功率 + `last_attempt_at` + 派生分桶（LEFT JOIN 保留区间内零尝试 channel）。金额一律十进制字符串不经 float。 |

新增内部包能力：`internal/service/admin/query` 扩 `RecoveryService`（List 脱敏 + Get include_internal）+ `ChannelHealthService`（派生分桶，阈值同 M9）；新增 `internal/app/adminapi/system.go` + 路由装配（`/system/*`）；`sql/queries/settlement_recovery_jobs.sql` 新增 `GetSettlementRecoveryJobByID`/`ListSettlementRecoveryJobsPage`（脱敏列）/`CountSettlementRecoveryJobs`，新增 `sql/queries/system.sql` 的 `SystemChannelHealth`（sqlc 已生成）；`admin_server.go` 复用同一 `sqlc.Queries` 装配两只读 service。单测：服务层（recovery 过滤透传/金额映射/include_internal 脱敏/not_found/非法 id、channel 健康分桶 + 区间透传）+ adminapi handler 状态码（列表不含内部详情、user_id 非法 400、详情 include_internal 透传、不存在 404、缺 token 401、channel-health 区间透传 + 非法时间 400）+ `store/sqlc` DB 门控 `TestSystemQueriesAgainstSchema`（recovery 列表/计数无过滤与全过滤、channel 健康有无区间两路径在真实 schema 上 well-formed）。前端 `unio-admin`：新增 `SystemPage`（结算补偿任务[筛选+分页+详情弹窗 `RecoveryJobDetailDialog`+脱敏开关] / 同步任务[复用 `listSyncJobs` 只读] / 渠道健康[区间预设 24h/7d/30d + 分桶分布 + 明细表] 三 tab），新增「运营」导航分组。

## 已交付（Slice 8，看板经营驾驶舱 D1 决策层重构，2026-06-18）

把 M9 工作台看板（Slice 6）从「数据展示页」升级为「经营驾驶舱」三层信息架构的第一片：首屏决策层。设计与口径以 [DESIGN-dashboard-business-overview.md](DESIGN-dashboard-business-overview.md) 为准，§9.1 口径已定稿并锚定 [DEC-021](../../production/DECISIONS.md)。

> **范围决策（本切片定调）**：D1 直接基于现有原表（不新增 migration、不引 rollup），低量即可上；环比用「本期 + 上期同长度紧邻窗口」两段同口径聚合，暂直扫原表，上量前迁 rollup（[GAP-13-001](../../production/TODO_REGISTER.md#gap-13-001)）。金额沿用 DEC-008/DEC-017「按币种分组、绝不跨币种相加」，**定稿为按币种拆卡、不引汇率**（DEC-021）。缓存贡献是反事实估算（非账本事实），UI 标「估算」。首屏只放 8 KPI + 环比 + 状态 Banner，**砍掉首页明细 / 排行 / Token 拆解**，降为后续二级中心（[GAP-13-002](../../production/TODO_REGISTER.md#gap-13-002)）。

| 任务 | 状态 | 说明 |
| --- | --- | --- |
| TASK-13.09（看板 D1 决策层重构） | done | **后端 service**（`internal/service/admin/dashboard/dashboard.go`+`helpers.go`）：`Overview` 重构为 `Current`/`Previous` 两段同口径环比（上期=同长度紧邻窗口）+ `Health` 整体健康状态 Banner（阈值 0.95/0.80）+ `MarginRate` 按币种派生利润率（`big.Rat` 精确：毛利/收入）+ 缓存贡献反事实估算（新增 `DashboardCacheSavingsByCurrency` 查询）。**adminapi DTO**（`internal/app/adminapi/dashboard.go`）：响应体改为 `current`/`previous`/`margin_rate`/`health` + 时点画像（balance/billing_exceptions/channels），↑↓% 由前端用 current/previous 自算。**前端**（`unio-admin`）：`dashboard.ts` 类型对齐；`DashboardPage.tsx` 重排为状态 Banner + 8-KPI 决策层（金额类带环比 `DeltaBadge`，异常/成本类 invert），保留趋势图与渠道健康，删除旧 `OverviewCards`/`MoneyList`。 |

8 KPI（经营优先级排列）：收入 / 毛利 / 利润率 / 缓存贡献（估）/ 请求数 / 成功率 / 异常请求 / 客户余额池。

修改文件：`sql/queries/dashboard.sql`（+`DashboardCacheSavingsByCurrency`，sqlc 已生成）；`internal/service/admin/dashboard/{dashboard,helpers}.go` + `dashboard_test.go`；`internal/app/adminapi/dashboard.go`；前端 `src/lib/api/dashboard.ts` + `src/pages/DashboardPage.tsx`。

## 验证（2026-06-18，看板 D1 决策层重构）

```bash
# 后端
go build ./...                                                      # 通过
go test ./internal/service/admin/dashboard/ ./internal/app/adminapi/  # 通过
# 前端（unio-admin）
npx tsc -b                                                         # 通过
npx eslint src/pages/DashboardPage.tsx src/lib/api/dashboard.ts    # 通过
npx vite build                                                     # 通过
```

新增 / 更新覆盖：dashboard service 单测迁到 `Current`/`Previous` 嵌套断言 + 补环比段非零、`MarginRate`（USD 8.2/12.5=0.656）、缓存贡献透传、整体 `Health=unhealthy`（含 unhealthy 渠道）断言；adminapi handler 状态码不变。后续切片 D2–D5 与移出本轮的三类无数据源指标已登记 [GAP-13-001~005](../../production/TODO_REGISTER.md)。
...
> 此前的环境性失败（`internal/core/modelcatalog.TestSyncAppliesAgainstDatabase`）已消除：本地开发库里那批 211 条手动 `seed_models_dev` 测试模型（零定价/绑定/成本快照依赖）已按用户确认清理（仅级联其能力位），保留 1 条手建 `manual` 模型；清理后该测试恢复绿色。

## 验证（2026-06-12，真实 Postgres，含 M8）

```bash
go build ./... ; go vet ./...                                       # 通过
DATABASE_URL=postgres://unio:***@localhost:5432/unio?sslmode=disable \
  REDIS_ADDR=localhost:6380 go test ./...                            # 全绿，0 失败（含此前清理后恢复的 modelcatalog 同步测试）
```

新增覆盖：query 包 `RecoveryService` 单测（status/user_id/from 过滤透传 + count 同条件、金额精确保留 scale、`include_internal` 默认脱敏/开启回显、`pgx.ErrNoRows`→`admin_not_found`、非法 id→`admin_invalid_argument`）、`ChannelHealthService` 单测（healthy/degraded/unhealthy/no_data 分桶 + 成功率 + `last_attempt_at` 透传 + 区间 narg 透传）、adminapi system handler 状态码（列表 body 不含 `last_internal_error_detail`、`user_id=abc`→400、详情 `include_internal` 透传、`/999`→404、缺 token→401、`channel-health` 区间透传 + 非法时间→400）、`store/sqlc` DB 门控 `TestSystemQueriesAgainstSchema`。前端 `unio-admin`：`bun run build`（tsc + vite）通过；`bun run lint` 仅余既有 shadcn 生成文件的 5 处告警，M8 新增文件零报错。

## 尚未开始 / 留尾

阶段 13 **主线切片已全部交付**（按 ADMIN_MODULES_DRAFT.md 推进顺序）：M1 认证（Slice 1）、M2 凭据 + M3 provider/channel/model/绑定（Slice 1/2）、M4 定价（Slice 2）、M6 只读查询台（Slice 3）、M7 客户/API Key 费用上限/手工调额（Slice 4）、M5 能力管理（Slice 5）、M9 工作台看板（Slice 6）、M8 系统/任务/健康横切（Slice 7）。原 GAP-6-005（项目级路由/预算闸门）已随 M7「项目=工作空间、费用上限挂 API Key」决策废弃。

留尾（独立切片，按需排期）：

| 留尾项 | 说明 |
| --- | --- |
| 熔断健康跨进程共享 + 后台手动恢复 | 当前熔断是 gateway 进程内内存态、admin 读不到实时电路；M8 channel 健康为 `request_attempts` 派生近似。要展示实时电路 + 一键恢复，需把熔断状态落到共享存储（Redis/DB）并开 admin 写动作（`breaker.go` 注释已将此列为阶段 13 能力）。 |
| M5 enforce DB runtime 热切 | 把 `CAPABILITY_ENFORCE_*` 从 env 挪成 DB runtime setting，避免 admin 显示与 gateway env 漂移；现状仍是改 env + 重启。 |
| models.dev 同步对既有模型能力刷新 | 当前同步只在新模型首次落库写粗能力。 |
| M9 大表聚合优化 | rollup/物化视图（草稿已预留 `usage_rollup_worker`）+ 跨时区分桶。看板经营驾驶舱已立项三层架构（[TASK-13.09](PLAN.md#task-13-09-dashboard-cockpit)）：D1 决策层已落地（Slice 8），D2 rollup 基础设施见 [GAP-13-001](../../production/TODO_REGISTER.md#gap-13-001)、二级中心 [GAP-13-002](../../production/TODO_REGISTER.md#gap-13-002)、实时页 [GAP-13-003](../../production/TODO_REGISTER.md#gap-13-003)。 |
