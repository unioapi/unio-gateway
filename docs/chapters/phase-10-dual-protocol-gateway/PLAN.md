...
状态：done（2026-06-02；adapter / gatewayapi / service 三层协议分离完成，lifecycle 抽取在 TASK-10.05 收口）
...
3. 【done，TASK-10.05】抽出协议无关 `service/gateway/lifecycle`，并在 OpenAI + Anthropic 两侧复用。
...
item 3（抽出协议无关 lifecycle）已在 TASK-10.05 完成，第二协议接入后再抽象的排序是正确的。
item 5（streamtranslate 收口）已在 TASK-10.07 完成，DeepSeek 无专属 stream framing，baseline 直接内联进 `openai.Adapter`。
```
...
状态：done（2026-06-02；事实契约、双协议 adapter 映射、schema、sqlc、settlement/recovery 与账务回归均已收口）
...
6. 【done，10.12A/10.12B】扩展 request、attempt、usage、price snapshot、cost snapshot、recovery job schema + `usage_line_items`，并完成本地库重建、`sqlc generate` 与测试回归。
7. 【done，10.12A/10.12B】settlement 与 recovery 改为只消费不可变 `ResponseFacts`/`usage.Facts`（`lifecycle/settlement*.go` + `billing`）；OpenAI 与 Anthropic 公开链路、recovery worker 和账务测试均已回归。

收口说明：item 1~5 是协议无关契约，item 6/7 已在 10.12A/10.12B 一次性完成 schema、sqlc、settlement/recovery 与账务回归。`adapter.ChatUsage` 只保留为 OpenAI 协议族内部响应解析和公开 usage DTO 映射的中间结构，不再作为账务事实输入。
...
状态：done（2026-06-02；请求 typed 全量、响应/stream 全字段贯通、DEC-012 Drop 语义与 usage facts 均已收口）
...
1. 【done】顶层请求字段全部登记并 typed 化；provider 不支持的合法协议字段按 DEC-012 在 adapter 出站 Drop。
2. 【done】messages 各 role、content union、多模态、audio、file、tool、deprecated function 字段显式处理。
3. 【done】response choices、message、logprobs、annotations、audio、tool_calls、usage details 完整建模。
4. 【done】stream chunk、delta、usage 尾包和 `[DONE]` 语义完整。
5. 【done】OpenAI vendor extension 只允许登记后 passthrough 或 provider mapping Drop。
6. 【done】图片、视频、音频、文件等能力字段只表示协议面可识别；当前 provider 不支持时在 adapter 出站 Drop，不因 provider 能力做 ingress 400。
