...
Unio 能识别、校验并交给 adapter 做显式策略，不代表当前 provider 或选中模型已经支持该能力。

Phase 10 只实现 Messages 对话链路，不扩展图片、视频、音频、文件等模型能力。
image、document、container upload 等 block 仍必须全量建模；若 DeepSeek 或后续
选中模型不能保持语义，必须在调用 tokenizer 或上游前明确 Reject。模型能力判断
后续应参考 [models.dev API](https://models.dev/api.json)、provider 官方文档和黑盒验收。
...
2. DeepSeek 不支持某 block，不代表 Unio HTTP DTO 可以静默忽略。
...
| `anthropic-beta` | `Typed` | 仅允许登记后的 beta；未知 beta 明确 Reject。 |
...
| `mcp_servers` extension | `Reject` | DeepSeek 兼容文档登记但会忽略；若未来作为扩展开放，必须单独登记 `AcceptIgnored` 策略。 |
...
3. provider 忽略 `budget_tokens` 时，必须按 DeepSeek mapping 明确 Reject 或登记为 ignored，不可悄悄丢失。
...
DeepSeek 对 server tool definition 没有逐项声明支持，必须在 provider mapping 中明确
Reject。注意 `server_tool_use` 与 `web_search_tool_result` content block 已由 DeepSeek
文档声明支持，两者不能混淆。
