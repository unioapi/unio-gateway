...
状态：in_progress（acceptance sign-off）

## 阶段判断

Phase 10 已完成 ADR、范围冻结、双协议 adapter 纵向实现与共享 lifecycle 全部接线；
DEC-012 协议为先 Drop（双侧）、TASK-10.12（facts schema + 账务测试回归）、TASK-10.10B-2b（Anthropic service/handler + 共享 attempt/delivery lifecycle，含 e2e parity）、TASK-10.11（双协议错误与安全输出）、TASK-10.15（命名/冗余复核）、TASK-10.13（OpenAI SDK 黑盒验收）、TASK-10.14（Anthropic SDK 黑盒验收）、TASK-10.10 durable closeout 终局与 **TASK-10.05 架构 B 终局泛型化（thin wrapper 收口，2026-06-02）** 均已收口。Phase 10 双协议公开 Gateway 已完成全栈实现 + 共享 lifecycle + SDK 黑盒回归 + durable closeout；当前阶段状态只剩 acceptance sign-off。可选回归（DS-OAI-05/06/08、DS-ANT-15、tools 多轮 reasoning 回传等）不作为 Phase 10 关闭阻断，排到后续回归窗口。
...
Phase 10 关闭前只剩签核动作，不再登记主线实现任务。签核时需要确认：
...
7. 最终验收命令通过后，把本文件状态与 `docs/PROJECT_STATUS.md` 阶段 10 状态改为 `done`。

## 下一步

建议下一节课直接做 **Phase 10 acceptance sign-off**：

1. 先跑轻量静态关闭检查（TODO/GAP、mapping Verify、runtime adapter 绑定、旧 OpenAI root DTO）。
2. 再跑 `go vet ./internal/... ./cmd/...`、`go build ./internal/... ./cmd/...`。
3. 如果本地 Postgres/Redis 可用，跑带 `DATABASE_URL` 的 `go test ./internal/...`。
4. 最后跑 `go test -tags=blackbox -count=1 ./internal/blackbox/openaisdk/...` 与 `./internal/blackbox/anthropicsdk/...`；真实 DeepSeek smoke 可按密钥窗口决定是否启用。
5. 全绿后更新阶段 10 为 `done`，再进入 Phase 11 的 credential resolver、admin auth 和 provider/channel 管理边界复核。
