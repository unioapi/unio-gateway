...
```text
7.20 成本价与毛利审计：先落地 channel_cost_prices 和 request-level cost_snapshots；第一版不做倍率系统。
```

建议接入顺序：

1. 复核剩余 P0 blocker，确认 `GAP-7-007` 仍暂不插队。
2. 进入 [GAP-7-009](../../production/TODO_REGISTER.md#gap-7-009)：先写 `channel_cost_prices` migration。
3. 再写 `cost_snapshots` migration，并保证请求级成本快照保存单价副本和实际成本金额。
4. 为两张表新增 `sql/queries/channel_cost_prices.sql` 和 `sql/queries/cost_snapshots.sql`。
5. 后续后台/报表查询需要同时读取 `ledger_billing_exceptions` 中的 `write_off` 与 `risk_exposure` 事件。
6. price effective window 放在 [GAP-7-010](../../production/TODO_REGISTER.md#gap-7-010)；settlement recovery 暂时不做，等进入 `cmd/worker` / recovery job 小节时再处理。
...
17. 下一步新增 `migrations/000019_create_channel_cost_prices.up.sql`
18. 下一步新增 `migrations/000020_create_cost_snapshots.up.sql`
19. 下一步新增 `sql/queries/channel_cost_prices.sql`
20. 下一步新增 `sql/queries/cost_snapshots.sql`
