...
| TASK-7.07 | partial | stream 有 final usage 时可 settlement，调用上游前 authorization baseline、部分余额授权、平台差额核销、无 final usage 风险敞口记录、request/attempt 状态机守卫、request-level settlement 成功重放和首次 settlement 失败 recovery 已接入；SSE 写出后错误观测仍由 [GAP-7-006](../../production/TODO_REGISTER.md#gap-7-006) 收口。 |
...
| 任务 | 状态 | 说明 |
| --- | --- | --- |
| 无 | - | 当前建议下一步处理 GAP-7-006 stream 写出后错误观测。 |

## 未完成

| 任务 | 状态 | 说明 |
| --- | --- | --- |
| GAP-7-006 后续观测 | todo | SSE 写出后无法再返回 JSON error，仍需在阶段 8 stream observability / SSE error event 线收口。 |
...
1. 确认当前阶段没有剩余 P0 release blocker。
