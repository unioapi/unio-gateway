...
| 任务 | 状态 | 说明 |
| --- | --- | --- |
| TASK-7.23 | P0 done | Stream partial settlement（路线 B/D）核心已落地：`usage.SourcePartialStreamEstimate`；partial 均写 `final_usage_received=false`，但生命周期终态按原因区分：用户取消 `canceled`、上游中断 `failed`、正常缺 usage `succeeded`；`BuildPartialStreamFacts` 合成审计 facts；输出 token 复用 adapter tokenizer 增量计数（OpenAI tiktoken / Anthropic 估算器）；`RunStreamGeneric` 与 `message_stream.go` 两处循环按 emitted 分流（B/D partial、C release），interrupt 重排为先 partial 结算再写 failed；B4 只看 `streamFacts`；旧三处 `risk_exposure` 调用已移除（保留 settlement-failed/dead-finalize）。OpenAI + Anthropic 单测覆盖 B(cancel/interrupt)、D、!emitted release、B4。 |
