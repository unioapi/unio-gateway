...
```text
阶段 13 前置小节，进入后台 channel 管理和凭据管理前。
```
...
10. 阶段 10 已把 runtime 绑定升级为 channel.protocol + channel.adapter_key；后台写入 registry 校验推迟到阶段 13 provider/channel CRUD。

计划实现：

1. 阶段 13 后台 provider/channel CRUD 写入 channel.protocol + channel.adapter_key 时校验 adapter 必须存在于对应协议 registry。
...
2. 阶段 13 项目策略管理处理 project 禁用、专属 channel 和后台配置入口；模型能力运行时闸门由阶段 12 capability architecture 接管。
