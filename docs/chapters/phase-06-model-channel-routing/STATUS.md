...
| TASK-6.03 | done | adapter registry、credential/routing、gateway/settlement、HTTP handler/rate limit、server app wiring 和启动期 preflight 已迁入 `internal/bootstrap`；阶段 10 已升级为 `channel.protocol + channel.adapter_key` 复合绑定校验，后台写入校验推迟到阶段 11 provider/channel CRUD。 |
