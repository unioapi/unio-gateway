...
- 修改边界：本轮只更新本报告，没有修改生产代码、数据库或 Blueprint 文档
...
- B-1 新增的清扫任务解决了“失败请求仍冻结余额”，但旧的孤儿清扫可能误伤正常长流，也存在与结算任务并发抢跑的窗口，因此不能算完全关闭。
- A-3 已经移除硬编码 token，会话实现本身合理；但公开登录入口没有失败次数限制，仍可被高速反复猜口令。
- 删除整个 `internal/blackbox` 虽然让 B-3/C-6 的失败消失了，也同时删除了真实上游和 P4 故障演练，不能把“删测试”当成“功能已再次证明正常”。
...
| B-1 预扣搁浅泄漏 | 部分修复，需重新打开 | 新 `stranded_reservation_sweeper` 已装配并有 worker 单测，但孤儿清扫仍有误收口和并发窗口，见 B-1R。 |
...
### B-1R 正常长流可能被当成“孤儿请求”清掉余额

**是否存在：存在。** 孤儿清扫只看“冻结超过 15 分钟、请求仍在 running、查询时没有 recovery job”。正常流式请求没有总时长上限，只要持续有输出就可以合法运行超过 15 分钟，因此也会满足这组条件。

**影响：** 清扫可能在客户仍接收内容时释放冻结余额并把请求标成失败，随后真正结算又会因为请求已失败而无法正常完成。列表查询后如果刚好新建了 recovery job，收口事务也不会再次确认，仍可能抢在结算前处理。

**建议怎么改：** 收口前在同一个事务中再次确认“没有 recovery job”，并增加“请求最近仍有活动”或“没有活跃 attempt/流”的判断。不要只按创建时间判断。至少补两个数据库测试：一个超过 15 分钟但仍有活动的长流不能被清扫；一个查询后并发创建 recovery job 时不能被清扫。
...
- 原 P4 状态丢失演练曾经通过 15/15，但当前套件已删除。重新建立可重复演练前，不应把旧结果当作当前版本的自动保证。
...
| blackbox / P4 / 真实上游 | 当前源码已删除对应套件，本轮未执行；旧手工结果不作为当前自动验证 |
| `deadcode` / `staticcheck` | 本机未安装，本轮未重新执行 |

结论：当前代码可以正常构建，普通单测、Admin 检查和文档校验均通过；但账务清扫边界、partial recovery、登录防猜测、长请求 TPM 和凭据批量暴露仍不适合仅靠运维规避，建议优先按本报告高风险项处理后再做上线确认。
