...
> **不允许**出现在本 provider 与其依赖的协议族 base adapter(`internal/core/adapter/openai/chatcompletions`)里——
> 那些只能存在于具体第三方 provider 的 adapter 层(如 `internal/core/adapter/openai/deepseek/chatcompletions`)。
...
| --- | --- | --- | --- | --- |
| OpenAI | `https://api.openai.com/v1` | `POST /chat/completions` | `internal/core/adapter/openai/chatcompletions`(base 直接复用) | `openai` |
...
| `developer` role | **忠实透传**(官方原生支持),不塌缩为 `system`(路线 C 已完成,2026-06-12;DeepSeek 塌缩已下沉 `openai/deepseek`) |
| `max_completion_tokens` | **忠实透传**为同名字段(官方新模型用它),不塌缩为 `max_tokens`(路线 C 已完成,2026-06-12;DeepSeek 塌缩已下沉 `openai/deepseek`) |
...
| base adapter | 协议族通用实现 `internal/core/adapter/openai/chatcompletions`:wire 编码、HTTP、响应解析、SSE、usage,**应是忠实的官方基线** |
