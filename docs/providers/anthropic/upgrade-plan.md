...
- ✅ **已落地并改造(2026-07-07)**:beta 转发由白名单严进改为**透传 + 小黑名单**(`beta.go`)——默认转发所有 beta,
  仅拦截有计费/解析缺口的 `code-execution-2025-05-22`、`context-1m-2025-08-07`。此前白名单曾漏 `extended-cache-ttl-2025-04-11`,
  导致 1h 缓存降级为 5m(用户实测证实),已随本次改造修复。
  **完整审计、逐项对照与后续改造(P1-A code-execution 计费维度等)见 [passthrough-audit.md](passthrough-audit.md)。** 本待查证项(首批 beta 集合)据此**关闭**。
- **优先级**:P0。**状态**:P0-A / P0-B 已完成;P1(code-execution 计费维度、structured-outputs 实测)待排期(见 audit §6)。
...
### N4 · `anthropic-version` 策略(P2,G3)✅ 已查证(2026-07-07)
- **现状**:base 硬编码 `2023-06-01`(`adapter.go` L20–22)。
- **结论**:`2023-06-01` 是官方**当前唯一稳定版**(`2023-01-01` 已废弃,官方 SDK 也硬编码此值)。钉死**正确,不需要改**,也不透传客户端版本头。详见 [passthrough-audit.md §9](passthrough-audit.md)。
- **降级为观察项**:官方若出新版本头且需支持其新特性,用 `app_settings` 机制加 `anthropic.api_version` 配置项即可(近零成本)。**状态**:观察。
