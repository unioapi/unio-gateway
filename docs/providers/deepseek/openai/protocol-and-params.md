# DeepSeek · OpenAI 格式 协议与参数映射

> 本文件是 DeepSeek **OpenAI 兼容格式**(`POST /chat/completions`)的**权威逐字段映射**
> (由 Phase 10 映射表迁入,出现分歧以此为准)。适配决策与转换逻辑见同目录
> [adaptation.md](adaptation.md);计费见 [../billing.md](../billing.md);
> 模型目录向 V4 迁移等待办见 [../upgrade-plan.md](../upgrade-plan.md)。
> 标准 OpenAI 协议本身见 [docs/protocol/openai/chat-completions/](../../../protocol/openai/chat-completions/official.md)。
