...
- **现状**:`function.strict` 字段**已端到端透传**——ingress typed DTO(`ChatFunctionTool.Strict`)→ contract
  → `internal/core/adapter/openai/chatcompletions/request_wire.go`(`Tools: marshalJSONValue(req.Tools)` 原样上送),无任何 Drop;
...
- **tokenizer 无 v4 失效**:仅估算输入;`internal/core/adapter/openai/chatcompletions/tokenizer.go` 的 `fallbackEncoding`
