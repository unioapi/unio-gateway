...
// luaControlReconcile 仅供停机重启的启动协调使用。PostgreSQL 是权威事实，因此无论
// Redis control 缺失、落后、超前、payload 漂移或遗留 pending，都原子替换为 DB 当前 active。
// 只重建目标 control key，不触碰独立的计数桶、租约、request token 或其它运行态 key。
// KEYS[1]=control。ARGV: revision, payload_hash, payload。返回 {'unchanged'|'reconciled'}。
const luaControlReconcile = `
local control = KEYS[1]
