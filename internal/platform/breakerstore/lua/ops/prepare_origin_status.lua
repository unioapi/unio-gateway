...
if tonumber(current) == nil or tonumber(next_rev) ~= tonumber(current) + 1 or not valid_status(next_eff) then
  return { 'invalid' }
end
if op_state == 'prepared' then
  if
    redis.call('HGET', ep, 'status_revision_state') == 'pending'
    and redis.call('HGET', ep, 'status_fence_token') == token
    and redis.call('HGET', ep, 'status_payload_hash') == payload_hash
    and redis.call('HGET', ep, 'pending_status_revision') == next_rev
    and redis.call('HGET', ep, 'pending_effective_status') == next_eff
  then
    return { 'prepared' }
  end
  return { 'conflict' }
