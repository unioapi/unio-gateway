...
//
//   - string：非空即视为有内容。
//   - 数组：逐个 part 结构化校验。text/refusal part 允许；image_url/input_audio/file 这类多模态 part
//     在当前 provider/模型为 text-only 的范围内统一前置 Reject（字段 typed 不等于能力支持，
//     后续按模型能力数据放开）。
//
// 非法结构或当前范围不支持的 part 返回 chatValidationError；调用上游前即拒绝，避免把无法保持语义的请求透传给上游。
	...
	for i, rawPart := range rawParts {
		...
		case contentPartTypeImageURL, contentPartTypeInputAudio, contentPartTypeFile:
			return messageContentState{}, &chatValidationError{
				...
				message: fmt.Sprintf("unsupported content part: %s", part.Type),
			}
		case "":
			return messageContentState{}, &chatValidationError{
				param:   fmt.Sprintf("%s.type", partParam),
