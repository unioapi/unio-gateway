...
// origins_handler.go 提供 /v1/responses 主创建上游源站之外的其余 origin handler：
//   - POST /responses/compact     无状态降级压缩（TASK-11.11 / GAP-11-007）
//   - POST /responses/input_tokens 本地估算 input token（TASK-11.12 / GAP-11-008）
//   - 有状态 origin（retrieve/delete/cancel/input_items）统一 501（TASK-11.13 / GAP-11-009）
//
...
// NewResponsesCompactHandler 构造 compact origin handler。
...
// NewResponsesInputTokensHandler 构造 input_tokens origin handler。
...
// NewResponsesStatelessUnsupportedHandler 构造有状态 origin 的统一 501 handler。
//
// retrieve/delete/cancel/input_items 共用同一处理：Unio 无服务端响应存储，统一返回
// 501 unsupported_origin_stateless（无需 service，纯静态响应）。
func NewResponsesStatelessUnsupportedHandler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		writeResponsesStatelessUnsupported(
			w,
			"This origin is not supported. Unio does not store responses; resend the full input on each request.",
		)
	})
}
