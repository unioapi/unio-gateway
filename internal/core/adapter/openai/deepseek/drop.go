...
func dropUnsupported(req openai.ChatRequest) (openai.ChatRequest, []string) {
	var dropped []string
...
func adaptUserID(req openai.ChatRequest) (openai.ChatRequest, bool) {
	if req.User == nil {
		return req, false
	}
...
func dropCustomTools(tools []openai.ChatTool) ([]openai.ChatTool, bool) {
	if len(tools) == 0 {
		return tools, false
	}

	kept := make([]openai.ChatTool, 0, len(tools))
	removed := false
...
func dropUnsupportedMessageParts(messages []openai.ChatMessage) ([]openai.ChatMessage, bool) {
	var cleaned []openai.ChatMessage
	removedAny := false
	...
	for i, msg := range messages {
		...
		if cleaned == nil {
			cleaned = append([]openai.ChatMessage(nil), messages...)
		}
		msgCopy := msg
