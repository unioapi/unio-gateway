...
//   - DEEPSEEK_BLACKBOX_BASE_URL，默认 https://api.deepseek.com；
//   - DEEPSEEK_BLACKBOX_MODEL，默认 deepseek-v4-flash；
//   - DEEPSEEK_BLACKBOX_REASONER_MODEL，默认 deepseek-v4-pro（DS-OAI-03/04）。
//
// 任一必填缺失即 skip，符合"黑盒用例作为可选回归保留"的约定。
func newBlackboxAdapter(t *testing.T) (*Adapter, channel.Runtime) {
	...
	baseURL := os.Getenv("DEEPSEEK_BLACKBOX_BASE_URL")
	if baseURL == "" {
...
func blackboxChatModel() string {
	if model := os.Getenv("DEEPSEEK_BLACKBOX_MODEL"); model != "" {
		return model
	}
	return "deepseek-v4-flash"
}

func blackboxReasonerModel() string {
	if model := os.Getenv("DEEPSEEK_BLACKBOX_REASONER_MODEL"); model != "" {
		return model
	}
	return "deepseek-v4-pro"
}

// skipUnlessDirectDeepSeekAPI 跳过仅官方 DeepSeek 端点保证的能力（logprobs、reasoning_content 分离等）。
// 经 OpenRouter 等代理实测时这些能力由承载方决定，不等价于 api.deepseek.com 黑盒基线。
func skipUnlessDirectDeepSeekAPI(t *testing.T) {
	t.Helper()
	...
	if !strings.Contains(baseURL, "api.deepseek.com") {
		t.Skipf("skipping DeepSeek-provider-specific blackbox on non-direct upstream %q", baseURL)
	}
}

func blackboxUserMessage(text string) chatcompletionsadapter.ChatMessage {
	content, _ := json.Marshal(text)
	return chatcompletionsadapter.ChatMessage{Role: "user", Content: content}
}
...
func TestDeepSeekBlackboxNonStream(t *testing.T) {
	...
	resp, err := adapter.ChatCompletions(context.Background(), runtime, chatcompletionsadapter.ChatRequest{
		Model:     blackboxChatModel(),
		Messages:  []chatcompletionsadapter.ChatMessage{blackboxUserMessage("Reply with the single word: ok")},
		MaxTokens: intPtr(8),
	})
	if err != nil {
...
func TestDeepSeekBlackboxStream(t *testing.T) {
	...
	var content string
	var finalUsage *chatcompletionsadapter.ChatStreamChunk
	outcome, err := adapter.StreamChatCompletions(context.Background(), runtime, chatcompletionsadapter.ChatRequest{
		Model:     blackboxChatModel(),
		Messages:  []chatcompletionsadapter.ChatMessage{blackboxUserMessage("Reply with the single word: ok")},
		MaxTokens: intPtr(8),
	}, func(chunk chatcompletionsadapter.ChatStreamChunk) error {
...
func TestDeepSeekBlackboxTokenizerCalibration(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:     "chinese",
			messages: []chatcompletionsadapter.ChatMessage{blackboxUserMessage("用一句话说明为什么计费系统必须保证幂等。")},
		},
	}
...
func TestDeepSeekBlackboxThinkingNonStream(t *testing.T) {
	...
	resp, err := adapter.ChatCompletions(context.Background(), runtime, chatcompletionsadapter.ChatRequest{
		Model:     blackboxReasonerModel(),
		Messages:  []chatcompletionsadapter.ChatMessage{blackboxUserMessage("What is 2+2? Reply with just the number.")},
...
// DS-OAI-RouteC：路线 C 下沉后 DeepSeek 全链路仍 200——ingress 侧 developer + max_completion_tokens
// 经 deepseek.Adapter 塌缩为 system + max_tokens 后上游可接受（与 wire_test 互补，走真实 HTTP）。
func TestDeepSeekBlackboxRouteCCollapseStill200(t *testing.T) {
	adapter, runtime := newBlackboxAdapter(t)
	...
	resp, err := adapter.ChatCompletions(context.Background(), runtime, chatcompletionsadapter.ChatRequest{
		Model: blackboxChatModel(),
		Messages: []chatcompletionsadapter.ChatMessage{
			{Role: "developer", Content: jsonMarshal(t, "You answer with exactly one word.")},
			blackboxUserMessage("Reply with the single word: ok"),
		},
		MaxCompletionTokens: &maxCompletionTokens,
	})
	if err != nil {
		t.Fatalf("ChatCompletions after route C collapse adapt: %v", err)
	}
	if resp.Content == "" {
		t.Fatal("expected non-empty content after developer/max_completion_tokens collapse")
	}
	if resp.Usage.PromptTokens <= 0 {
		t.Fatalf("expected positive prompt tokens, got %+v", resp.Usage)
	}
