...
func adaptLegacyFunctions(req chatcompletionsadapter.ChatRequest) (chatcompletionsadapter.ChatRequest, []string) {
	var dropped []string
...
func convertLegacyFunctions(raw json.RawMessage) ([]chatcompletionsadapter.ChatTool, bool) {
	var defs []legacyFunctionDef
	if err := json.Unmarshal(raw, &defs); err != nil || len(defs) == 0 {
		return nil, false
	}

	tools := make([]chatcompletionsadapter.ChatTool, 0, len(defs))
	for _, def := range defs {
		if strings.TrimSpace(def.Name) == "" {
			return nil, false
		}
		tools = append(tools, chatcompletionsadapter.ChatTool{
			Type: "function",
