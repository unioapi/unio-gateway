...
func TestFindActivePriceForModelFiltersAndOrders(t *testing.T) {
	...
	olderActive := priceParams(modelID, now)
	olderActive.InputPrice = numeric(2)
	olderActive.EffectiveFrom = timestamptz(now.Add(-2 * time.Hour))
	createPriceForTest(t, ctx, queries, olderActive)

	newerActive := priceParams(modelID, now)
	newerActive.InputPrice = numeric(3)
	newerActive.EffectiveFrom = timestamptz(now.Add(-30 * time.Minute))
	want := createPriceForTest(t, ctx, queries, newerActive)
