...
-- 基线字典种子（与历史 scripts/seed-capability-keys.sql 合并进本迁移）：
--   * 空库 migrate 后即可用，无需再跑手工种子脚本。
--   * ON CONFLICT DO NOTHING：已存在的 key 不覆盖（不影响线上已改文案）。
--   * 后续新增能力 = 追加新迁移 INSERT … ON CONFLICT DO NOTHING，或在本段末尾追加（仅未发布库）。
